...
export default function HomeClient() {
  ...
  return (
    <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                ...
                <Link key={i} href={item.href} className="p-12 rounded-[40px] bg-white/5 border border-white/5 hover:border-white/20 transition-all group">
                  <h3 className="text-3xl font-black italic uppercase mb-4 group-hover:text-[#FF3B00] transition-colors" style={{ fontFamily: 'Georgia, serif' }}>
                    {item.title}
                  </h3>
                  <p className="text-white/40 italic font-medium" style={{ fontFamily: 'Georgia, serif' }}>
                    {item.desc}
                  </p>
                </Link>
              ))}
      ...
      <AnimatePresence>{osMode && <JamesOS key="jamesOS" onExit={exitOS} />}</AnimatePresence>
    </>
  );
}
