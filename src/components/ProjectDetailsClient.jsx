...
import { FiArrowLeft, FiExternalLink, FiGithub, FiChevronLeft, FiChevronRight, FiX, FiMaximize2 } from 'react-icons/fi';
import { normalizeImageUrl } from '@/lib/imageUtils';

const ACCENT_COLORS = {
  'Web Development': '#FF3B00',
  'Mobile App': '#00E5FF',
  'UI/UX Design': '#AAFF00',
  'Branding': '#FF0099',
  'E-commerce': '#FFD700',
  'AI / Machine Learning': '#7B61FF',
  default: '#6366f1',
};
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="overflow-hidden mb-10">
          <motion.h3
            style={{ y: titleYSpring, fontFamily: 'Georgia, serif' }}
        ...
        <motion.div
          style={{ opacity: contentOpacity, y: contentY, fontFamily: 'Georgia, serif' }}
          className="max-w-3xl"
        >
          <p className="text-xl md:text-2xl text-white/50 leading-relaxed whitespace-pre-line">{content}</p>
        </motion.div>
      </div>
    </motion.div>
  );
      ...
      {images.length > 1 && (
        <>
          <button
            ...
            className={`absolute left-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/10 backdrop-blur-md text-white border border-white/20 transition-all ${
          ...
          <button
            ...
            className={`absolute right-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/10 backdrop-blur-md text-white border border-white/20 transition-all ${
              currentIndex === images.length - 1 ? 'opacity-0' : 'opacity-0 group-hover:opacity-100'
            }`}
          >
    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="relative w-full h-full flex items-center justify-center">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative w-full h-full">
            <Image
...
export default function ProjectDetailsClient({ project }) {
  ...
  const techUsed = project.technologies
    ...
    <main className="min-h-screen bg-white">
          ...
          <div className="max-w-7xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] }}>
              <div className="flex items-center gap-4 mb-8">
                ...
                <span className="text-[10px] tracking-[0.3em] uppercase text-white/50 font-mono">{project.year}</span>
              </div>
              <h1 className="text-7xl md:text-[9rem] font-black uppercase text-white leading-[0.8] tracking-tight mb-4 max-w-5xl" style={{ fontFamily: 'Georgia, serif' }}>
                {project.title}
              </h1>
            </motion.div>
          </div>
      ...
      <section className="py-12 bg-white border-b border-black/5">
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex flex-col md:flex-row justify-between items-center gap-12">
          <div className="flex flex-wrap gap-2 md:max-w-2xl">
            {techUsed.map((tech, i) => (
              <span key={i} className="text-[10px] md:text-[11px] uppercase tracking-widest border border-black/10 px-4 py-2 rounded-full font-mono text-black/60 hover:bg-black hover:text-white transition-all cursor-default">
                {tech}
              </span>
            ))}
          </div>
          <div className="flex gap-4 w-full md:w-auto">
            {project.projectUrl && (
              <a href={project.projectUrl} target="_blank" rel="noopener noreferrer" className="group flex flex-1 items-center justify-between gap-4 px-8 h-14 bg-black text-white rounded-full hover:scale-105 transition-transform">
            ...
            {project.githubUrl && (
              <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="group flex items-center justify-center w-14 h-14 border border-black/10 rounded-full hover:bg-black hover:text-white transition-all">
                <FiGithub size={20} />
              </a>
            )}
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-12 flex justify-between items-end">
            <div>
              ...
              <h2 className="text-4xl font-bold italic" style={{ fontFamily: 'Georgia, serif' }}>
                The Gallery
              </h2>
            </div>
            <div className="text-[10px] font-mono text-black/20 uppercase tracking-widest">{allImages.length} Images / Project Showcase</div>
          </div>
          <motion.div className="w-full" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            <GalleryCarousel images={allImages} projectTitle={project.title} onImageClick={(i) => setModalIndex(i)} accent={accentColor} />
          </motion.div>
        </div>
      </section>
      ...
      <section className="bg-[#080808] text-white">
        {sections.map((section, index) => (
          <DetailSection key={index} index={index} total={sections.length} title={section.title} content={section.content} accent={accentColor} />
        ))}
      </section>

      <section className="bg-[#080808] pt-12 pb-32 px-6">
        <div className="max-w-7xl mx-auto border-t border-white/5 pt-24 text-center">
          <h2 className="text-5xl md:text-[10rem] font-black italic mb-12 uppercase text-white leading-[0.8] tracking-tighter" style={{ fontFamily: 'Georgia, serif' }}>
            Interested? <br />
            <span className="block mt-6" style={{ color: accentColor }}>
              Let&apos;s work together.
            </span>
          </h2>
          <Link href="/#contact" className="inline-flex items-center gap-4 bg-white text-black px-12 py-5 rounded-full font-mono text-xs uppercase tracking-[0.2em] hover:scale-105 transition-transform">
            Get In Touch
          </Link>
        </div>
      </section>
